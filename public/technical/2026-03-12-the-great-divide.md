...
* Whether or not I can already hear a Danfo [^3] conductor belting an alternatively favourable route I could take. 
...
Keep in mind that although the equation derived assumes that our vectors, $\mathbf{a}$ and $\mathbf{b}$, are both two-dimensionsonal vectors, the formula still scales to vectors of higher dimensions. Indeed, it is on this premise that the perceptron builds its central formula. 
...
[^5]: (There was Eid, so the number was smaller, but I still don't want to truncate)
