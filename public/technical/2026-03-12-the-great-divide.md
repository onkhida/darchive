...
Practically speaking, our perceptron model will make a recursive run (or technically, an epoch) through my data points, checking each individual point's classification along the way (via its dot product) and adjusting its weight vector for each misclassification. It will only stop this process if it is able to complete an epoch without making any misclassifications, since it is only at that point where it would have aligned its weight vector as a compass, capable of separating the binary categories into each of its halves.
