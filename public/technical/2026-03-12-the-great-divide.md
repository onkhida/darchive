...
# Solid Foundations
When I was a child, the most grounding aspects of reality came within the binary. A capacity for nuance came (as it does for most people) later on, but initially, I was interested in seeing whether something could be, for example, hot or cold, right or wrong, hard or soft, bitter or sweet, or interesting or boring. My satisfaction with a word depended on whether it had an opposite, and trying to understand "why" yields itself to a very simple explanation: it is foundational. 

As intelligent beings, although our ability to tolerate and operate within the grey is innate, it mostly ever actualises after going through a gestation period. And even then, these binary logic systems remain—sometimes dormantly—in the lowest level of our programming, to be reignited whenever we need them[^1].

If the binary can provide a solid enough architectural base for even children to build on top of, it makes sense to start the quest for artificial intelligence at its doorstep. Following this line of thinking, there has been many an algorithm over the years that has attempted to equip machines with the ability to duly make these categorical decisions. The most enduring of these has its origins in Frank Rosenblatt's *"Mark I Perceptron"*, and this foundational layer, upon which modern AI and neural networks now build, has formed the basis for this post. I've dedicated much of my efforts with this entry towards providing a systematic breakdown of how Rosenblatt's original algorithm works as the foundational piece of engineering that it is. I'll go through each iterative step, explaining finally (and in a heavily analogical manner) how this all culminates to form a basic computational system capable of making binary conclusions when confronted with a series of relevant data points (vectors). 

# On Vectors
The last word in my introductory section is "vectors", in parentheses. I've been deliberate in putting this mathematical concept in our atmosphere very early on because, apart from its use in underpinning the core mechanism upon which the perceptron is built, thinking of our inputs as vectors is both useful and intuitive. For coming to any decision, even a simple (binary) one, is usually contingent on a multitude of different factors. 
...
Keep in mind that although the equation derived assumes that our vectors, $\mathbf{a}$ and $\mathbf{b}$, are both two-dimensional vectors, the formula still scales to vectors of higher dimensions. Indeed, it is on this premise that the perceptron builds its central formula. 
...
Regardless of whatever representation of the weighted sum, $z$, that you use (with $w_0x_0$, $w_0$, or $b$ as the bias term), because the $z$ simply represents the dot product between the weight vector and a point, the sign $(+ve/-ve)$ of the result of this sum, $z$, can now finally tell us to which half that a data point, $x$, belongs to according to the weight vector's current orientation. 

<div class="interactive-component" data-component="weighted-sum-visualizer" data-props="%7B%7D"></div>

The job of the perceptron from this step would then be to check if this classification coincides with that of the training data, reorienting if that is not the case.

## The Offset Operator

The reason we even have a bias term in the first place is that it gives our decision boundary an offset so that it does not always run through the origin. To inspect the impact of this bias term geometrically, imagine that we had a scenario where the perceptron was working with only vectors of two dimensions. In a scenario like this, if we wanted to represent this decision boundary graphically on the Cartesian plane, we would simply be looking for the line where $z=0$. That is, where

$$w_yy+w_xx+b=0$$
(assuming $w_x$ and $w_y$ referred to the $x$ and $y$ components of our derived weight vector respectively)

If we were to go even further and solve for $y$, the decision boundary would be given by

$$y=-\frac{w_x}{w_y}x-\frac{b}{w_y}$$

Rearranging these terms into the conventional $y=mx+c$ form reveals two important things to us. First, it is that the gradient of this decision boundary is defined by $-\frac{w_x}{w_y}$. The second is that the y-intercept of our line can, in turn, be distinctly defined by $-\frac{b}{w_y}$.

So whilst the weight vector can reorient and point in different directions by adjusting the values of its components (its gradient, after all, is $-\frac{w_x}{w_y}$), a bias term just allows us to move the orthogonal decision boundary it generates "up" or "down" the weight vector. In the component below, you can try to play around with different values of $w_0$, $w_1$, and $w_2$. I'd recommend that you specifically view a weight vector (and thus its decision boundary) with and without a value for $w_0$ (that is, without a bias term) to fully grasp what the bias does for us.
...
Although we have a [heavy conceptual proof](https://web.engr.oregonstate.edu/~huanlian/teaching/ML/2024fall/unit2/convergence.html) backing this update rule, it is easy to see on a conceptual level why this equation suffices as an iterative corrective mechanism. Remember that if a point is misclassified, then the dot product, $\vec{w}\cdot\vec{x}$, simply has the wrong sign. Because of this, adding $yx$ to the existing weight shifts our weight (and its decision boundary) towards the direction of the misclassified point, increasing the likelihood that it is categorised correctly on the next iteration!
...
Practically speaking, our perceptron model will make a run through my data points, checking each individual point's classification (via its dot product) and adjusting its weight vector for each misclassification along the way. This run (or pass) through our training data is called an "epoch", and it is only when our perceptron algorithm can complete one full _epoch_ without making any misclassifications that it will stop iterating. If it is able to run a pass without misclassifications, that means that every point is (by definition of its current decision boundary) in the half that it is meant to be; it would have aligned its weight vector as a compass, capable of separating the binary categories into each of its halves.

Whenever you're ready, you can press the play button on the simulation below. The blue line is the live weight vector, the dashed line is its orthogonal decision boundary, and the points on the graph are the different decision categories to be separated. You can pause, play, and skip through epochs with the control on the side pane, but really, what you should also be looking at is the perceptron's *"Boundary Diaries"*. As it converges on a solution, the *"Boundary Diaries"* on the side pane will work as a log, providing the requisite mathematical solutions of the steps it is taking in applying the algorithm. If the process feels too fast or too slow, you can also click the stopwatch icon at the bottom right of the component to adjust the speed at which the perceptron does its checks and reorients its weight vector through the iterations.

<div class="interactive-component" data-component="perceptron-simulator" data-props="%7B%7D"></div>

As per the visualisation above, our final weight vector is $\vec{w} = (2.045, -1.563)$ with a bias ($w_0$) of $0.400$. The "x-component" of our weight vector, which we also refer to as $w_1$ in the visualisation, is the weight we've derived for our money variable, whilst the "y-component" of our weight vector ($w_2$) is the weight we derived for our "time" variable. Here's how our weighted sum, $z$, would look now with these updated values.

$$z=2.045x_1-1.563x_2+0.400$$

If we were to look only at the absolute values of these weights, we could make a **tentative** inference that, because $w_1 > w_2$, the amount of money present in my card was a stronger variable for consideration than my perceived waiting time to hop on a bus. But now that we have a relative idea of the strength of influence of these variables, I suppose the next thing one can be curious about is of _how specifically_ these variables influence my decision process. To answer that question, we'll just have to look at the sign of the respective weights: a positive weight for money means that the more money present in my card, the more likely it is that I board a BRT bus. Contrarily, the longer my perceived wait time for a bus is, the less likely it is that I board the BRT bus. 

Why is this so? Because in our context, all data points that have a positive weighted sum (or dot product) are classified as instances where I should be taking the BRT; the data points that yield a negative weighted sum are classified as instances where I should not be taking the BRT. So since our weight for money, $w_1$, is positive, then increasing the amount of money ($x_1$) I have "adds" to this weighted sum, in turn increasing the chances that I take the bus ($z>0$). The opposite is true for our weight for the waiting time. Because $w_2$ is negative, then the longer waiting times (larger $x_2$) end up making significant subtractions from our final weighted sum, $z$. This reduces the chances that I decide to board a bus.

The bias is important in our context, too. I could infer, from our positive bias of 0.4, that, all things being equal, I am very likely to take the BRT. It's telling us that if (hypothetically) I had no money $(x_1=0)$ and didn't have to wait for the bus ($x_2=0$), my weighted sum would still be positive—it would still be an instance categorised as favourable for bus taking. To put it in other words, I have a default inclination to take the BRT.

In the component below, click to plot a point on the graph. You will receive a classification based on which side of the boundary your point falls on. Alternatively, you can also type in an amount of money (in NGN) and time (in minutes) that I could hypothetically have in a decision instance and see, based on that data, whether I will take a BRT or not, given the circumstances.

<div class="interactive-component" data-component="decision-classifier" data-props="%7B%22w0%22:0.4,%22w1%22:2.045,%22w2%22:-1.563%7D"></div>

# Winter Was Coming
At the end of the "Checks and Balances" section, I promised (rather bullishly) that the perceptron "*will* converge on a solution, just as long as the points are linearly separable." My confidence in the perceptron's ability to always find a hyperplane does not emanate from anywhere within me; there is an elucidatory, elegant convergence theorem written 60-odd years ago by academics much more accomplished than I currently am. I do not intend on descending into this convergence theory in this post, but what I've only thought to highlight is that the conditional that the convergence theory rests on—"...as long as the points are linearly separable"—turned out to be a limitation large enough to usher in the first AI winter in the 1970s. The world has always been more complex than linear classifications of yes/no, and this initial model I've just presented found it difficult to scale to more complex applications. On some occasions, the problems at hand didn't even need to be complex: [single-layer perceptrons struggled to solve simple XOR problems](https://direct.mit.edu/books/monograph/3132/PerceptronsAn-Introduction-to-Computational).

<div class="interactive-component" data-component="xor-problem" data-props="%7B%7D"></div>

But even if the perceptron only ever had linearly separable data to work with, we have no certitude that the hyperplane it finds will be the "correct" one, for an arbitrary definition of "correct". In fact, if you scroll back up to the simulation above, you would see that there is a point (representing March 25) that teeters incredibly close to the brink of our finalised decision boundary. The point for March 25 is labelled as $-1$ (did not take BRT) and its dot product with the final weight vector is $-0.001$. So though we may struggle to see it with our naked eye, because $y(\vec{w}\cdot\vec{x})$ is actually a positive number $(-1\times-0.001=0.001)$, it is _just about_ classified correctly. 

The March 25 point is perfectly illustrative of a particular feature (and perhaps flaw) of rudimentary perceptron algorithms: it never actively sought to find a margin between points/groups; its process spun around iteratively trying to ensure that each point fell on the right side of the boundary, regardless of how close to a dotted line a point may have been. And it is points like March 25, which find themselves hovering around the boundary, that reveal why researchers eventually sought to find algorithms that separated with a greater degree of confidence than the perceptron initially inspired. For example, more developed techniques like Support Vector Machines (SVMs) seek to (instead of finding a random hyperplane) find a hyperplane that maximises the margin between data classes.

Yet, despite all these initial difficulties, humanity remains grateful for its inception. Its theory has formed the basis for much of what we've discovered regarding the development of intelligence artificially. Even though Rosenblatt's initial design was a single-layer, linear classifier, it paved the way for the more complex multi-layer perceptron models, which, as we were to find out across the years, developed into special types of neural networks. It has propagated—literally, through algorithms of similar learning, and metaphorically, as a foundational symbol of mathematical thinking—an atomic, incremental progression in our development. We owe much to it.

# Coda
This entry metamorphosed, in part, from notes I took at the turn of the year after reading Anil Ananthaswamy's ["Why Machines Learn"](https://www.amazon.com/Why-Machines-Learn-Elegant-Behind/dp/0593185749). My father bought the paperback last year and liked it, so I gave it a try. Ananthaswamy also talks about Rosenblatt's perceptron in the first two chapters, and although it's a concept I encountered (in length) beforehand, his text stands as (by far) my favourite contemporary take on the concept. Throughout the book, his conceptual framing goes beyond core technicalities, and he finds unique ways of masterfully weaving in the relevant bits of situational, social commentary to make the reader feel as though they are living through the developmental timeline and experiencing the creation of our modern systems in real time. It is the next [consumable](http://onkhida.me/consumables) that I am to write about, and if you have even a remote interest in the science underpinning current technology, the book will be worth your time.

But the main accelerator for this post was, of course, my daily commute. After making iterative decisions on whether or not to take a BRT bus, I was interested in seeing (as leisure) if a perceptron algorithm could (hypothetically) learn from the actions I took recursively and figure out the main principles guiding my daily decisions. I knew that I was organically considering variables in my head repeatedly, and you could say that I was interested in finding out what each variable _weighed_ in my decision process. The plan initially was to manually gather the "data" for every decision instance en route to work each day in the month of March, logging values for all the consequential variables I could identify (fatigue, patience levels, money in Cowry Card, lateness, etc.). Usually, as I develop, I create build notes of what is being created. But somewhere along the way on this occasion, the prose morphed into a narrative explanation of the processes guiding this experiment, and thus the perceptron. 

I've since accepted this directional pivot in turning this into a technical article. The only caveat is that, because my words came together much faster than my experimental, month-long inquest for data, the figures I've tabularised and used on the plot have been pulled arbitrarily from my head to fit into the central example context. I've also limited the hypothetical to two variables, because a 2D graph makes for much easier visualisations. And hopefully, despite this shift in my writing aims, this has still been helpful. I've had fun attempting to make this make sense, and if there be any questions, feedback, or suggestions regarding how this was written, I'm forever reachable via [email](mailto:daniel.eta@outlook.com).
...
[^5]: Nigeria's Federal Government has just announced that the 19th and 20th of this month will be public holidays due to this year's Eid celebrations. So, technically, it will be 20 work days; I've just persisted with 22 because this entry was essentially done when the announcement came in.