...
The reason we even have a bias term in the first place is because it gives our decision boundary an offset so that it does not always run through the origin. To inspect the impact of this bias term geometrically, imagine that we had a scenario where the perceptron was working with only vectors of two dimensions. In a scenario like this, if we wanted to represent this decision boundary graphically on the cartesian plane, we would simply be looking for the line where $z=0$. That is, where
...
In the component below, click to plot a point on the graph. You will receive a classification based on what side of the boundary your point falls on. Alternatively, you can also type in an amount of money (in NGN) and time (in minutes) that I could hypothetically have in a decision instance and see, based on that data, whether I will take a BRT or not given the circumstances.
...
> Make a graph of our Perceptron

For as you can see in the graph above, although we've only plot four points across the graph in (0,0), (0,1), (1,0), and (1,1), its impossible to draw a straight line separating the categories. (0,1) and (1,0), the only inputs that yield a 1 in an XOR device, can't be duly separated from the other points, (0,0) and (1,1), that yield 0.
...
The March 25 point is perfectly illustrative of a particular feature (and perhaps flaw) of rudimentary perceptron algorithms: it never actively sought to find a margin between points/groups; it's process spun around iteratively trying to ensure that each point fell on the right side of the boundary, regardless of how close to a dotted line a point may have been. And it is points like March 25, that find themselves hovering around the boundary, that reveal why researchers eventually sought to find algorithms that separated with a greater degree of confidence than the perceptron initially inspired. For example, more developed techniques like Support Vector Machines (SVMs) seek to (instead of finding a random hyperplane) find a hyperplane that maximises the margin between data classes.

Yet, despite all these initial difficulties, humanity remains grateful for its inception. It's theory has formed the basis for much of what we've discovered regarding the development of intelligence artificially. Even though Rosenblatt's initial design was as a single-layer, linear classifier, it has paved the way for the more complex multi-layer perceptron models, which, as we were to find out across the years, developed into special types of neural networks. It has propogated—literally, through algorithms of similiar learning, and metaphorically, as a foundational symbol of mathematical thinking—an atomic, incremental progression in our development. We owe much to it.
